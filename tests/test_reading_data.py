...
from src.reading_data import reading_from_exel, reading_from_csv


def test_reading_from_exel():
    """ Функция для тестирования reading_from_exel"""
    mock_data = pd.DataFrame({
        'id': [3107343, 2130098, 4653427],
        'state': ['EXECUTED', 'CANCELED', 'EXECUTED']
    })

    with patch('pandas.read_excel', return_value=mock_data):
        result = reading_from_exel('any_path.xlsx')
